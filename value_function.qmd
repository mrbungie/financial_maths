...
For this decomposition, we know there are different period interest rates (look at the example in Basic Notions):
...
```{python}
#| echo: false
#| fig-cap: "Comparison of Value Functions for Two Decompositions"
#| label: fig-value-function-comparison

import pandas as pd
from plotnine import ggplot, aes, geom_line, geom_point, labs, theme_bw, scale_x_continuous, scale_y_continuous, theme

# Data in tidy format
data = {
    "time": [0, 1, 2, 3, 0, 1, 2, 3],
    "value": [1000, 1046.67, 1119.93, 1276.72,
              1000, 1010, 1026.16, 1500.25],
    "decomposition": ["Decomposition 1"] * 4 + ["Decomposition 2"] * 4,
}
df = pd.DataFrame(data)

# Plot with plotnine
p = (
    ggplot(df, aes(x="time", y="value", color="decomposition"))
    + geom_line(size=1.2)
    + geom_point(size=3)
