...
**Slide:** Financial Mathematics Slides 1-96, Slides 34-46 (Value function definition, properties, and relationship with interest rates), Page 34, [Direct Link](https://cdn.jsdelivr.net/gh/mrbungie/financial_maths@main/resources/slideshows/25_09_30_FinancialMathematics_Slides_1_96.pdf#page=34)
:::

## Introduction

Suppose we have a financial transaction on two dates, represented as:
...
The financial transaction is: $\{-1000, 1050\} / \{0, 1\}$
...
Let's now extend this notion to multi-period financial transactions.

Consider the following **composite transaction**:

- At time 0, an amount $S$ is invested by a financial institution
- At time 1, the debtor can decide to:
  1. **Pay the entire sum**, with interest $I$. The resulting financial operation is:
     $$\{-S, S+I\} / \{0, 1\}$$
  2. **Defer the principal reimbursement** for one year, paying an additional interest $I'$, so that the operation becomes:
     $$\{-S, S+I+I'\} / \{0, 2\}$$
...
- Total payment: €10,500 at year 1

**Option 2**: Defer payment for another year, but pay an **additional €600 interest**

- Total payment: €11,100 at year 2 (original €500 + extra €600)
...
However, we can see the second choice as a **composition** of two basic operations:

$$
\{-S, S+I\} / \{0, 1\} \oplus \{-(S+I), (S+I)+I'\} / \{1, 2\}
$$

Therefore, this contract involves **three different time periods**: this implicitly defines an intertemporal equivalence law $V(t)$ on three dates, whose representation is:
...
Using our student loan example:

| $t$ (years) | $V(t)$ (€) | What's happening |
|-------------|------------|------------------|
| 0           | 10,000     | Loan received    |
...
For the value function to make economic sense, we impose two fundamental assumptions:
...
To see this with an example, let's go back to the investment/borrowing operation:

$$
\{1000, -380, -380, -380\} / \{0, 1, 2, 3\}
$$

Let's suppose that interest is described by **Decomposition 1**, such that:

$$
\{1000, -333.33-46.67, -333.33-46.67, -333.34-46.66\} / \{0, 1, 2, 3\}
$$

where each payment is decomposed into principal and interest.
...
We can now express some key quantities in a more formal way, in terms of the value function.
...
Suppose that a transaction is summarized by the value function in this table:
...
Just as we can express interest in terms of the value function, we can also define discount-related quantities.
...
- **Interest** (forward-looking): "If I invest today, how much will I **gain**?"
...
- **Specific financial laws** that define how the value function evolves:
  - Simple interest (linear growth)
  - Compound interest (exponential growth)
  - Continuous compounding
  
- **Practical applications**:
  - How to compute value functions for real financial products
  - Solving for unknown interest rates or time periods
  - Converting between different interest rate conventions
...
The value function is the **foundation**—everything else builds on this!
:::
